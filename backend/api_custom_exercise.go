...
func (app *App) handleAddCustomExercise(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		...
		VALUES ($1, $2, NULLIF($3, ''), NULLIF($4, ''), $5::jsonb, NULL, NULLIF($6, ''), NULL, $7::jsonb, $8::jsonb, '[]'::jsonb)
	`
	_, err := app.db.Exec(query, id, req.Name, strings.TrimSpace(req.Category), strings.TrimSpace(req.Equipment), string(primaryMusclesJSON), strings.TrimSpace(req.Level), string(instructionsJSON), string(secondaryMusclesJSON))
	if err != nil {
