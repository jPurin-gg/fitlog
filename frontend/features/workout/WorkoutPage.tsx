...
import { AlternativeCoachModal } from "@/features/exercises/components/AlternativeCoachModal";
import { ExerciseSelectorModal } from "@/features/exercises/components/ExerciseSelectorModal";
...
import { finishWorkoutSession, getSetRecommendation, getSummaryComment, getTargetSets, recordWorkoutSet, saveManualWorkoutPlan, startWorkoutPlan, updateTargetSets } from "./api";
import type { SetRecommendation, WorkoutPlanSession } from "./types";
...
export default function WorkoutPage() {
  ...
  React.useEffect(() => {
    const fetchTargetSets = async () => {
      ...
      try {
        const data = await getTargetSets(currentExercise.id);
  ...
  const saveTargetSets = async (value: number) => {
    ...
    try {
      await updateTargetSets(currentExercise.id, value);
    } catch { /* 無視 */ }
  };
  ...
  const startWorkout = async () => {
    ...
    try {
      const data = await startWorkoutPlan(formatLocalDate(new Date()));
  ...
  const startManualWorkout = async (exercise: { id: string; name: string }) => {
    ...
    try {
      ...
      await saveManualWorkoutPlan(date, exercise);
      const data = await startWorkoutPlan(date);
      applyStartedWorkout(data);
    } catch (e) {
      console.error(e);
  ...
  const fetchRecommendation = async (workoutID: number, setID: number) => {
    ...
    setRecommendationError(null);
    try {
      const data = await getSetRecommendation(workoutID, setID, controller.signal);
  ...
  const saveCurrentSet = async () => {
    ...
    setRecommendationError(null);
    try {
      const recorded = await recordWorkoutSet(workoutID, setRequestRef.current.key, setInput);
  ...
  const fetchSummaryComment = async (workoutID: number) => {
    ...
    setSummaryCommentError(null);
    try {
      const data = await getSummaryComment(workoutID, controller.signal);
  ...
  const finishWorkout = async () => {
    ...
    try {
      ...
      const workoutID = workoutPlan.workout_id;
      const data = await finishWorkoutSession(workoutID);
      recommendationAbortRef.current?.abort();
      activeSetIDRef.current = null;
