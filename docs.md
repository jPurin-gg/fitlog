...
| username   | TEXT NOT NULL             | ニックネーム      |
| password_hash | TEXT                   | PBKDF2-SHA256 形式のパスワードハッシュ |
| created_at | TIMESTAMPTZ DEFAULT NOW() |                 |

> ログイン時にニックネームとパスワードの組み合わせが既存ユーザーに一致しない場合、新しいユーザーとして作成する。同じニックネームでもパスワードが異なれば別ユーザーになりうる。
...
> パスワードは平文保存せず、ランダムsalt付き PBKDF2-SHA256 ハッシュとして `users.password_hash` に保存する。フロントエンドはログイン結果の `user_id` と `nickname` を `localStorage` に保持し、各APIに `user_id` として渡す。
...
### `GET /api/calendar`
カレンダー表示用。指定月の完了済みワークアウト実施日と、日付から履歴詳細へ遷移するための `workout_id` を返す。同じ日に複数のワークアウトがある場合は、その日の最新ワークアウトを代表として返す。
...
- 実施済みの日付を押すと `/workouts/[id]` のワークアウト履歴詳細へ遷移する
...
| 項目                  | 現状                                       | 改善案                                  |
|--------------------|------------------------------------------|----------------------------------------|
| ログイン              | ニックネーム + パスワードでユーザー作成 / ログイン | セッション管理、ログアウト以外のアカウント管理 |
